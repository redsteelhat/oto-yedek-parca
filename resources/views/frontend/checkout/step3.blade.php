...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-center space-x-4">
            ...
            <div class="flex items-center">
                <div class="flex items-center justify-center w-10 h-10 rounded-full bg-primary-600 text-white font-bold">3</div>
                <span class="ml-2 font-semibold text-primary-600">Ödeme</span>
            </div>
                ...
                <form action="{{ route('checkout.storeStep3') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        @if(isset($paymentMethods['credit_card']))
                            <label class="flex items-start p-4 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('payment_method', $selectedPaymentMethod) == 'credit_card' ? 'border-primary-500 bg-primary-50' : '' }}">
                        ...
                        @if(isset($paymentMethods['bank_transfer']))
                            <label class="flex items-start p-4 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('payment_method', $selectedPaymentMethod) == 'bank_transfer' ? 'border-primary-500 bg-primary-50' : '' }}">
                        ...
                        @if(isset($paymentMethods['cash_on_delivery']))
                            <label class="flex items-start p-4 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('payment_method', $selectedPaymentMethod) == 'cash_on_delivery' ? 'border-primary-500 bg-primary-50' : '' }}">
