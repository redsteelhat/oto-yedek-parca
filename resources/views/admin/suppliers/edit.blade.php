...
@section('title', 'Tedarikçi Düzenle: ' . $supplier->name)
@section('page-title', 'Tedarikçi Düzenle')
    ...
    <form action="{{ route('admin.suppliers.update', $supplier) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="url" name="xml_url" value="{{ old('xml_url', $supplier->xml_url) }}"
                       class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                @error('xml_url')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="xml_type" required
                        class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                    <option value="standard" {{ old('xml_type', $supplier->xml_type) == 'standard' ? 'selected' : '' }}>Standart</option>
                    <option value="custom" {{ old('xml_type', $supplier->xml_type) == 'custom' ? 'selected' : '' }}>Özel</option>
                </select>
                @error('xml_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="xml_username" value="{{ old('xml_username', $supplier->xml_username) }}"
                       class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                @error('xml_username')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="password" name="xml_password" value=""
                       class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
            ...
            <div>
                ...
                <input type="number" name="update_frequency" value="{{ old('update_frequency', $supplier->update_frequency) }}" min="1" required
                       class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
            ...
            <div>
                <label class="flex items-center mt-6">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $supplier->is_active) ? 'checked' : '' }} class="mr-2">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="3"
                          class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">{{ old('notes', $supplier->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
